  ...
  return (
    <div className="min-h-screen" style={{ background: "#F7FAF9", paddingTop: "104px" }}>
      ...
      <div className="container-xl py-10 md:py-14 flex flex-col gap-14 md:gap-16">
        ...
        <div id="specialistai" className="scroll-mt-32"><SL>{c.specialistsLabel}</SL><ST>{c.specialistsTitle}</ST><div className="grid sm:grid-cols-2 max-w-[320px] gap-5">{c.specialists.map((s) => (<div key={s.slug} className="bg-white rounded-2xl border border-[#DDE9E8] overflow-hidden hover:border-[#90CECA] hover:shadow-[0_8px_32px_rgba(144,206,202,0.12)] transition-all duration-300 flex flex-col"><a href={`/specialistai/${s.slug}`} className="block h-[300px] bg-gradient-to-br from-[#EEF5F4] to-[#D4EDEB] overflow-hidden"><img src={s.photo} alt={s.name} className="w-full h-full object-cover" style={{ objectPosition: "center 10%" }} /></a><div className="p-5 flex flex-col flex-1"><a href={`/specialistai/${s.slug}`} className="hover:text-[#7DB9B5] transition-colors duration-200"><p className="text-[1rem] font-bold text-foreground mb-1 leading-snug">{s.name}</p></a><p className="text-[0.8125rem] font-medium text-[#7DB9B5] mb-3 leading-snug">{s.role}</p><p className="text-[0.8125rem] text-muted leading-relaxed flex-1">{s.focus}</p><a href={`/specialistai/${s.slug}#registruotis`} className="mt-4 inline-flex items-center gap-1.5 text-[0.8125rem] font-semibold text-[#7DB9B5] hover:text-[#68A7A2] hover:gap-2 transition-all duration-200">{c.specialistCta} <ArrowRight size={14} strokeWidth={2.5} /></a></div></div>))}</div></div>
